				...
				(
					...
					from (
						...
						(
							...
							from pg_type
							inner join pg_namespace ON pg_type.typnamespace = pg_namespace.oid
							where pg_type.typtype = 'b' and pg_type.typname = ('_' || c.udt_name) and pg_namespace.nspname='public'
							limit 1
						)
						order by pg_enum.enumsortorder
					) as labels
...
func (p PostgresDriver) Imports() (importers.Collection, error) {
	...
	col.BasedOnType = importers.Map{
		...
		"types.JSON": {
			ThirdParty: importers.List{`"github.com/themontem/sqlboiler/v4/types"`},
		},
		"types.Decimal": {
			ThirdParty: importers.List{`"github.com/themontem/sqlboiler/v4/types"`},
		},
		...
		"pgeo.Point": {
			ThirdParty: importers.List{`"github.com/themontem/sqlboiler/v4/types/pgeo"`},
		},
		"pgeo.Line": {
			ThirdParty: importers.List{`"github.com/themontem/sqlboiler/v4/types/pgeo"`},
		},
